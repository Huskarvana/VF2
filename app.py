...
def fetch_rss_articles(rss_list, lang):
    ...
    for url in rss_list:
        ...
        for entry in feed.entries:
            titre = entry.title
            lien = entry.link
            if "ds" not in titre.lower():  # ne garde que si DS mentionné
                continue
...
def fetch_newsapi_articles(query, lang, limit=20):
    if not NEWSAPI_KEY or lang not in NEWSAPI_LANGUAGES:
        return []
    api = NewsApiClient(api_key=NEWSAPI_KEY)
    try:
        res = api.get_everything(q="DS Automobiles", language=lang, sort_by='publishedAt', page_size=limit)
        return [{
            "titre": a["title"],
            "lien": a["url"],
...
if st.button("🕵️ Lancer la veille"):
    ...
    for lang in selected_langs:
        ...
        all_articles += fetch_newsapi_articles(query=keyword or "DS Automobiles", lang=lang)
